...
<head>
    ...
    <title>MedSyst — Emergency Queue</title>
    @include('partials._styles')
    <style>
        .queue-header-band { background: var(--red); padding: 10px 2rem; display: flex; align-items: center; gap: 10px; }
        .queue-header-band p { font-size: 0.82rem; font-weight: 600; color: #fff; }
        .queue-header-band span { font-size: 0.75rem; color: rgba(255,255,255,.75); }

        
        .q-num-badge { display: inline-flex; align-items: center; justify-content: center; min-width: 52px; height: 36px; border-radius: 8px; background: var(--red-bg); color: var(--red-dark); font-family: 'DM Serif Display', serif; font-size: 1.1rem; font-weight: 700; }
...
<body>
<div class="shell">
    @include('partials._staff_sidebar', ['active' => 'queue.emergency'])
    ...
    <div class="main">
        ...
        <div class="queue-header-band">
            ...
            <p>EMERGENCY QUEUE <span>— Patients are ordered by arrival time. All are critical priority.</span></p>
        </div>
        ...
        <div class="content">
            <div class="page-header">
                <div class="page-header-left">
                    ...
                    <p>{{ $queues->total() ?? 0 }} critical patient(s) requiring immediate attention</p>
                </div>
            ...
            <div class="table-card">
                ...
                <div style="overflow-x:auto;">
                    <table class="data-table" id="queueTable">
                        ...
                        @endphp
                        <tr class="row-critical" data-status="{{ $queue->status }}" data-name="{{ strtolower($fullName) }}">
                            <td><span class="q-num-badge">{{ $queue->queue_number }}</span></td>
                            <td>
                                <div class="patient-cell">
                                    <div class="pt-avatar red">{{ $initials }}</div>
                                    <div>
                            ...
                            <td style="max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="{{ $patient->condition ?? '' }}">
                                {{ Str::limit($patient->condition ?? 'N/A', 40) }}
                            </td>
                            <td>
                                @if($queue->doctor)
                                    <span style="font-size:0.82rem;font-weight:500;">{{ $queue->doctor->name }}</span>
                                @else
                                    <button class="btn btn-ghost btn-sm" onclick="openAssign({{ $queue->id }})">
                                        <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
                                        Assign
                                    </button>
                                @endif
                            ...
                            <td>
                                <div style="font-size:0.8rem;color:var(--dark);font-weight:500;">{{ $waitMins }}m ago</div>
                            ...
                            <td>
                                <div class="action-group" style="justify-content:flex-end;">
                                    ...
                                    @endif
                                    @if($queue->doctor && $queue->status !== 'done')
                                    <button class="btn btn-ghost btn-sm" onclick="openAssign({{ $queue->id }}, {{ $queue->doctor_id }})">
                        ...
                        <tr><td colspan="7">
                            <div class="empty-state">
                                ...
                                <p>No emergency patients at this time.</p>
                            </div>
                        </td></tr>
                        @endforelse
                        </tbody>
...
<script>
    ...
    function applyFilters() {
        ...
        document.querySelectorAll('#queueTable tbody tr[data-name]').forEach(row => {
            ...
            row.style.display = (matchName && matchS) ? '' : 'none';
        });
    }
    function openAssign(queueId, currentDoctorId = null) {
        document.getElementById('assignForm').action = `/staff/queue/${queueId}/assign`;
        document.getElementById('assignDoctorSelect').value = currentDoctorId ?? '';
        document.getElementById('assignModal').classList.add('open');
    }
    document.getElementById('assignModal').addEventListener('click', e => {
        if (e.target === document.getElementById('assignModal')) e.target.classList.remove('open');
    });
</script>
</body>
</html>